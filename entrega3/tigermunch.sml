...
let
	...
	fun munchStm s = 
		case s of
			...
			| EXP (TEMP _) 						=> ()
			| tigertree.MOVE (TEMP t,CONST i) 	=> emit (OPER {assem = "movq $"^ITS(i)^", %'d0\n",src=[], dst= [munchExp (TEMP t)], jump = NONE})
			| tigertree.MOVE (MEM(e1),CONST i) 	=>  emit (OPER {assem = "movq $"^ITS(i)^", (%'d0)\n",src=[], dst= [munchExp e1], jump = NONE})
			...
			| tigertree.MOVE (TEMP i,e2) => emit (OPER {assem="movq %'s0, %'d0\n",src=[munchExp e2],dst=[i],jump=NONE})
	...
	and munchExp e = case e of
						...
						| BINOP(PLUS,TEMP t,e1) => result (fn r=> (emit (OPER {assem = "movq "^t^", %'d0\n",src=[t],dst=[r],jump=NONE});
						                                          (emit (OPER {assem ="addq %'s0, %'d0\n",src=[munchExp e1], dst=[r],jump=NONE}))))
						| BINOP(MUL,TEMP t,e1) => result (fn r=> (emit (OPER {assem = "movq "^t^", %'d0\n",src=[t],dst=[r],jump=NONE});
						                                         (emit (OPER{assem="imul %'s0, %'d0\n",src=[munchExp e1], dst=[r],jump=NONE}))))		 
						| BINOP(MINUS,TEMP t,e1) => result (fn r=> (emit (OPER {assem = "movq "^t^", %'d0\n",src=[t],dst=[r],jump=NONE});
						                                          (emit (OPER {assem ="subq %'s0, %'d0\n",src=[munchExp e1], dst=[r],jump=NONE}))))
						(*AGREGAR BINOP (_,e1,cte) y viceversa*)	   
						| BINOP (PLUS,e,CONST i) => result (fn r => (emit (OPER {assem="movq %'s0+"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE})))                                      
						| BINOP (MINUS,e,CONST i) => result (fn r => (emit (OPER {assem="movq %'s0-"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE})))
						| BINOP (MUL,e,CONST i) => result (fn r => (emit (OPER {assem="movq %'s0*"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE})))
						| BINOP (PLUS,CONST i,e) => result (fn r => (emit (OPER {assem="movq %'s0+"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE})))
						| BINOP (MINUS,CONST i,e) => result (fn r => (emit (OPER {assem="movq "^ ITS(i)^"- %'s0, %'d0\n",src=[munchExp e],dst=[r],jump=NONE})))
						| BINOP (MUL,CONST i,e) => result (fn r => (emit (OPER {assem="movq %'s0*"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE})))												
						
						| BINOP(PLUS,e1, TEMP t) => result (fn r=> (emit (OPER {assem = "movq "^t^", %'d0\n",src=[t],dst=[r],jump=NONE});
																	emit (OPER{assem="addq %'s0, %'d0\n",src=[munchExp e1], dst=[r],jump=NONE})))
						| BINOP(MINUS,e1, TEMP t) => result (fn r=> (emit (OPER{assem="subq %'s0, %'d0\n",src=[munchExp e1], dst=[r],jump=NONE});
						                                           (emit (OPER {assem = "movq "^t^", %'d0\n",src=[t],dst=[r],jump=NONE}))))                                           
						| BINOP(MUL,e1, TEMP t) => result (fn r=> (emit (OPER{assem="imul %'s0, %'d0\n",src=[munchExp e1], dst=[r],jump=NONE});
						                                           (emit (OPER {assem = "movq "^t^", %'d0\n",src=[t],dst=[r],jump=NONE}))))                                           
						
						| BINOP(PLUS,MEM e,CONST i) =>  result(fn r =>emit (OPER {assem =  "movq (%'s0)+"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE}))
						| BINOP(MINUS,MEM e,CONST i) =>  result(fn r =>emit (OPER {assem =  "movq (%'s0)-"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE}))	                                           
						| BINOP(MUL,MEM e,CONST i) =>  result(fn r =>emit (OPER {assem =  "movq (%'s0)*"^ ITS(i)^", %'d0\n",src=[munchExp e],dst=[r],jump=NONE}))
	...
	and munchArgs ((_,[]) : (int * exp list)) : unit = ()
		| munchArgs (n,((TEMP x)::xs)) 		= ((if (n<6) then emit (OPER {assem = "movq %'s0, "^(natToReg n)^"\n",src=[munchExp (TEMP x)],dst=[natToReg n],jump=NONE}) 
