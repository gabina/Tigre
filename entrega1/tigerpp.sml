...
fun ppTipo (TUnit) = print ("TUnit\n")
    | ppTipo (TNil) = print ("TNil\n")
    | ppTipo (TInt) =  print ("TInt\n")
    | ppTipo (TString) = print ("TString\n")
	| ppTipo (TArray (r,_)) =  (print ("TArray "); ppTipo (!r))
	| ppTipo (TRecord (l,_)) = (print ("TRecord "); (List.map (fn(s,r,i) => (print s ; ppTipo (!r) ; print (","))) l) ; ())
	| ppTipo (TTipo (str)) = (print ("TTipo ") ; print (str))    
     

fun ppenvEntry (VIntro) = print ("VIntro\n")
	|ppenvEntry (Var({ty = t})) = (print ("Var (") ; ppTipo(t) ; print (")"))
	|ppenvEntry (Func({level = _,label=l,formals=fl,result=r,extern=_})) = (print ("Func {level = (),label= "); print (l); map ppTipo fl; ppTipo r; print("}"))
	

fun exprAst e =(ppexpr ppstrm e;flush_ppstream ppstrm;TextIO.output(TextIO.stdOut, "\n"))

(* Falta hacerlo
fun tabPrint fa fb tab = 
*)
